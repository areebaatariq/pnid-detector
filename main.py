...
app = FastAPI(title="P&ID Symbol Detection API", version="1.4")
...
def detect_symbols_tiled(detector, image_path, tile_size=1024, overlap=100):
    ...
    for y in range(0, h, tile_size - overlap):
        for x in range(0, w, tile_size - overlap):
            ...
            tile_path = f"temp_tile_{uuid.uuid4().hex}.png"
...
# =====================================================
MODEL_DIR = "models"
MODEL_PATH = os.path.join(MODEL_DIR, "hensis.t7")
...
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(
        ...
        f"Please place 'hensis.t7' inside the 'models/' directory before starting the server."
    )
...
async def detect_and_visualize(file: UploadFile = File(...)):
    ...
    result_path = f"result_{uuid.uuid4().hex}.png"
    detector.visualize_detections(image_path, detections, save_path=result_path)
    ...
    # Return PNG directly (browser can display it)
    return FileResponse(
        path=result_path,
